...
def draw_feature_map(model, img_path, save_dir):
    ...
    # 1.这里需要改model，让其在forward的最后return特征图。我这里return的是一个Tensor的tuple，每个Tensor对应一个level上输出的特征图。
    i = 0
...
def main():
    ...
    parser.add_argument('--work_dir', default='work_dirs/TransFPN/faster_rcnn_swin_top_scale_spatial_xview')
    ...
    args.save_dir = os.path.join(args.work_dir, 'feature')
    os.makedirs(args.save_dir, exist_ok=True)
    args.checkpoint = os.path.join(args.work_dir, 'epoch_30.pth')
    args.img = 'data/xview/images/99.tif'
