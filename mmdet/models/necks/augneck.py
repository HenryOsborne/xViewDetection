...
class AugNeck(nn.Module):
    def __init__(self,
        ...
        self.high_lateral_conv.extend(
            [nn.Conv2d(mid_channels[-1], mid_channels[-1], (1, 1)) for k in
             range(len(self.adaptive_pool_output_ratio))])
        self.high_lateral_conv_attention = nn.Sequential(
            nn.Conv2d(mid_channels[-1] * (len(self.adaptive_pool_output_ratio)), mid_channels[-1], (1, 1)), nn.ReLU(),
            nn.Conv2d(mid_channels[-1], len(self.adaptive_pool_output_ratio), (3, 3), padding=(1, 1)))

        if self.use_path_augment:
            self.low_lateral_conv = nn.ModuleList()
            self.low_lateral_conv.extend(
                [nn.Conv2d(mid_channels[0], mid_channels[0], (1, 1)) for k in
                 range(len(self.adaptive_pool_output_ratio))])
            self.low_lateral_conv_attention = nn.Sequential(
                nn.Conv2d(mid_channels[0] * (len(self.adaptive_pool_output_ratio)), mid_channels[0], (1, 1)),
                nn.ReLU(),
                nn.Conv2d(mid_channels[0], len(self.adaptive_pool_output_ratio), (3, 3), padding=(1, 1)))
    ...
    def forward(self, inputs):
        ...
        h, w = p5.size(2), p5.size(3)
        # Ratio Invariant Adaptive Pooling
        AdapPool_Features = [F.interpolate(self.high_lateral_conv[j](F.adaptive_avg_pool2d(p5, output_size=(
        ...
        if self.use_path_augment:
            p5_fusion = p5 + adap_pool_fusion
            p5 = self.lateral_convs[0](p5_fusion)
            ...
            # ----------------------------------------------------------------------------------------------------------
            p2_clone = p2.clone()
            h, w = p2_clone.size(2), p2_clone.size(3)
            AdapPool_Features = [F.interpolate(self.low_lateral_conv[j](F.adaptive_avg_pool2d(p2_clone, output_size=(
                max(1, int(h * self.adaptive_pool_output_ratio[j])),
                max(1, int(w * self.adaptive_pool_output_ratio[j]))))),
                                               size=(h, w), mode='bilinear', align_corners=True) for j in
                                 range(len(self.adaptive_pool_output_ratio))]
            Concat_AdapPool_Features = torch.cat(AdapPool_Features, dim=1)
            fusion_weights = self.low_lateral_conv_attention(Concat_AdapPool_Features)
            fusion_weights = torch.sigmoid(fusion_weights)
            adap_pool_fusion = 0
            for i in range(len(self.adaptive_pool_output_ratio)):
                adap_pool_fusion += torch.unsqueeze(fusion_weights[:, i, :, :], dim=1) * AdapPool_Features[i]
            p2_fusion = p2 + adap_pool_fusion
            # ----------------------------------------------------------------------------------------------------------

            p2 = self.out_layer_p2(p2_fusion)
            ...
            p5_fusion = p5 + adap_pool_fusion
            p4 = F.interpolate(self.lateral_convs[0](p5_fusion), size=c4.size()[2:], **self.upsample_cfg) + c4
            p4 = self.cbam1(p4)
...
if __name__ == '__main__':
    ...
    neck = AugNeck(in_channels=[96, 192, 384, 768], out_channels=256, use_path_augment=True)
    y = neck(x)
    print(y)
