...
class FPN_GLOBAL(FPN):
    def __init__(self,
        ...
        self.ensemble_feature_2 = nn.Conv2d(256 * 2, 256, kernel_size=3, stride=1, padding=1)
        self.ensemble_feature_3 = nn.Conv2d(512 * 2, 512, kernel_size=3, stride=1, padding=1)
        self.ensemble_feature_4 = nn.Conv2d(1024 * 2, 1024, kernel_size=3, stride=1, padding=1)
        self.ensemble_feature_5 = nn.Conv2d(2048 * 2, 2048, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, c2, c3, c4, c5, c2_ext=None, c3_ext=None, c4_ext=None, c5_ext=None,
        ...
        if mode == 3:
            ...
            print("c2_ext size", c2_ext.size())
            print("c3_ext size", c3_ext.size())
            print("c4_ext size", c4_ext.size())
            print("c5_ext size", c5_ext.size())
            ...
            c2_fusion = self.ensemble_feature_2(torch.cat((c2, c2_ext), dim=1))
            c3_fusion = self.ensemble_feature_3(torch.cat((c3, c3_ext), dim=1))
            c4_fusion = self.ensemble_feature_4(torch.cat((c4, c4_ext), dim=1))
            c5_fusion = self.ensemble_feature_5(torch.cat((c5, c5_ext), dim=1))

            fusion_inputs = [c2_fusion, c3_fusion, c4_fusion, c5_fusion]

            fpn_out = super(FPN_GLOBAL, self).forward(fusion_inputs)

            bfp_inputs=[fpn_out[0],fpn_out[1],fpn_out[2],fpn_out[3]]

            bfp_outputs=self.bfp_global(bfp_inputs)

            return [bfp_outputs[0],bfp_outputs[1],bfp_outputs[2],bfp_outputs[3],F.max_pool2d(bfp_outputs[3], 1, stride=2)]
...
class FPN_LOCAL(FPN):
    def __init__(self,
        ...
        self.smooth_0 = nn.Conv2d(256 * self.fold, 256, kernel_size=3, stride=1, padding=1)  # 128
        self.smooth_1 = nn.Conv2d(256 * self.fold, 256, kernel_size=3, stride=1, padding=1)  # 128
        self.smooth_2 = nn.Conv2d(256 * self.fold, 256, kernel_size=3, stride=1, padding=1)  # 128
        self.smooth_3 = nn.Conv2d(256 * self.fold, 256, kernel_size=3, stride=1, padding=1)  # 128
    ...
    def forward(self, c2, c3, c4, c5, c2_ext=None, c3_ext=None, c4_ext=None, c5_ext=None,
        ...
        if mode == 2:
            ...
            print("c2  ", c2.size())
            print("c2_ext  ", c2_ext[0].size())
            print("c3  ", c3.size())
            print("c3_ext  ", c3_ext[0].size())
            ...
            print("c4  ", c4.size())
            print("c4_ext  ", c4_ext[0].size())

            print("c5  ", c5.size())
            print("c5_ext  ", c5_ext[0].size())

            print("ps0_ext  ", ps0_ext[0].size())
            print("ps1_ext  ", ps1_ext[0].size())
            print("ps2_ext  ", ps2_ext[0].size())
            print("ps3_ext  ", ps3_ext[0].size())
            '''

            fusion_c2 = self.fusion_layer4(
                torch.cat([c2] + [F.interpolate(c2_ext[0], size=c2.size()[2:], **self._up_kwargs)], dim=1))
            fusion_c3 = self.fusion_layer3(
                torch.cat([c3] + [F.interpolate(c3_ext[0], size=c3.size()[2:], **self._up_kwargs)], dim=1))
            fusion_c4 = self.fusion_layer2(
                torch.cat([c4] + [F.interpolate(c4_ext[0], size=c4.size()[2:], **self._up_kwargs)], dim=1))
            fusion_c5 = self.fusion_layer1(
                torch.cat([c5] + [F.interpolate(c5_ext[0], size=c5.size()[2:], **self._up_kwargs)], dim=1))
            ...
            bfp_local_inputs=[fusion_feature[0],fusion_feature[1],fusion_feature[2],fusion_feature[3]]


            bfp_local=self.bfp_local(bfp_local_inputs)

            p2 =bfp_local[0]   #fusion_feature[0]
            p3 =bfp_local[1]   #fusion_feature[1]
            p4 = bfp_local[2]  #fusion_feature[2]
            p5 = bfp_local[3]  #fusion_feature[3]

            ''' 
            print("p2 size",p2.size())  #200
            print("p3 size", p3.size()) #100
            print("p4 size", p4.size()) #50
            print("p5 size", p5.size()) #25

            print("ps0_ext[0] size", ps0_ext[0].size()) #53
            print("ps1_ext[0] size",ps1_ext[0].size())  #27 
            print("ps2_ext[0] size", ps2_ext[0].size()) #13
            print("ps3_ext[0] size", ps3_ext[0].size()) #7
            '''
            

            #inputs = [p2, p3, ps0_ext[0], ps1_ext[0]] 200 100 53 27   2
            #inputs=[p3,p4,ps1_ext[0], ps2_ext[0]]    # 100 50 27 13  3
            #inputs = [p2, ps0_ext[0], ps1_ext[0], ps2_ext[0]]  # 200 53 27 13

            #inputs = [p2, p3, ps2_ext[0], ps3_ext[0]]   #200  100 13 7  #6
            #inputs = [p2, p3, ps1_ext[0], ps2_ext[0]]  #200  100 27 13  #7
            #outputs=self.bfp(inputs)

            #return [p2, p3, p4, p5, F.max_pool2d(p5, 1, stride=2)]
            #return [outputs[0], outputs[1], outputs[2], p5, F.max_pool2d(p5, 1, stride=2)]
            ...
            p2_fusion = self.smooth_0(
                torch.cat([p2] + [F.interpolate(ps0_ext[0], size=p2.size()[2:], **self._up_kwargs)], dim=1))  #

            p3_fusion = self.smooth_1(
                torch.cat([p3] + [F.interpolate(ps1_ext[0], size=p3.size()[2:], **self._up_kwargs)], dim=1))  #

            p4_fusion = self.smooth_2(
                torch.cat([p4] + [F.interpolate(ps2_ext[0], size=p4.size()[2:], **self._up_kwargs)], dim=1))  #

            p5_fusion = self.smooth_3(
                torch.cat([p5] + [F.interpolate(ps3_ext[0], size=p5.size()[2:], **self._up_kwargs)], dim=1))  #

            return [p2_fusion,p3_fusion,p4_fusion,p5_fusion,F.max_pool2d(p5_fusion, 1, stride=2)]
            ''' 
            bsf_inputs=[p2_fusion,p3_fusion,p4_fusion,p5_fusion]
            bsf_outputs=self.bfp(bsf_inputs)
            return [bsf_outputs[0], bsf_outputs[1], bsf_outputs[2], bsf_outputs[3], F.max_pool2d(bsf_outputs[3], 1, stride=2)]
            '''
...
class GLNET_FPN_COLLABORATIVE_BFP(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 num_outs,
                 ):
        super(GLNET_FPN_COLLABORATIVE_BFP, self).__init__()
        ...
        self.ensemble_fpn_0 = nn.Conv2d(256 * 2, 256, kernel_size=1, stride=1, padding=1)
        self.ensemble_fpn_1 = nn.Conv2d(256 * 2, 256, kernel_size=1, stride=1, padding=1)
        self.ensemble_fpn_2 = nn.Conv2d(256 * 2, 256, kernel_size=1, stride=1, padding=1)
        self.ensemble_fpn_3 = nn.Conv2d(256 * 2, 256, kernel_size=1, stride=1, padding=1)
    ...
    def init_weights(self):
        ...
        self.resnet_global.init_weights('modelzoo://resnet50')  # 'torchvision://resnet50'

        self.fpn_global.init_weights()

        self.resnet_local.init_weights('modelzoo://resnet50')  # 'torchvision://resnet50'

        self.fpn_local.init_weights()

        #self.bfp.init_weights()

        self.bfp_collaborative.init_weights()
    ...
    def _crop_global(self, f_global, top_lefts, ratio):
        ...
        for i in range(b):
            ...
            # print("global feature subregion",top, left)
    ...
    def collect_local_fm(self, image_global, patches, ratio, top_lefts, oped, batch_size, global_model=None,
        ...
        with torch.no_grad():
            ...
            if self.patch_n == 0:
                ...
                self.ps0_g, self.ps1_g, self.ps2_g, self.ps3_g, self.ps4_g = global_model.module.fpn_global.forward(
                    self.c2_g, self.c3_g, self.c4_g, self.c5_g, mode=1)
            ...
            # print("self.c2_g",self.c2_g.size())
            # print("ps0_g",self.ps0_g.size())

            ps0, ps1, ps2, ps3, ps4 = self.fpn_local.forward(c2, c3, c4, c5,
                                                             self._crop_global(self.c2_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.c3_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.c4_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.c5_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.ps0_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.ps1_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.ps2_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio),
                                                             self._crop_global(self.ps3_g, top_lefts[oped[0]:oped[1]],
                                                                               ratio), mode=2, )
            # print("self.patch_n ",self.patch_n )
            ...
            self.ps00_b = self._merge_local(ps0, self.ps00_b, self.ps0_g, top_lefts, oped, ratio, template)
            ...
            self.ps10_b = self._merge_local(ps1, self.ps10_b, self.ps1_g, top_lefts, oped, ratio, template)
            ...
            self.ps20_b = self._merge_local(ps2, self.ps20_b, self.ps2_g, top_lefts, oped, ratio, template)
            ...
            self.ps30_b = self._merge_local(ps3, self.ps30_b, self.ps3_g, top_lefts, oped, ratio, template)
            ...
            if self.patch_n == 0:
                ...
                self.ps30_l.append(self.ps30_b);
                ...
                # self.ps3_b = []  # ; self.output_b = []
                #print("self.c2_l",self.c2_l)
                #print("self.ps30_l",self.ps30_l)
            ...
            if len(self.c2_l) == batch_size:
                ...
                # print("s self.ps00_l",self.ps00_l.size())
                ...
                #print("before cat",self.ps10_l[0].size())
                ...
                #print("after cat, self.ps10_l ",self.ps10_l.size())
                ...
                self.ps30_l = torch.cat(self.ps30_l, dim=0)  # .cuda()

                #print("self.ps30_l ",self.ps30_l .size())

                self.ps0_l = self.ps00_l,  # [self.ps00_l, self.ps01_l, self.ps02_l, self.ps03_l]
                self.ps1_l = self.ps10_l  # [self.ps10_l, self.ps11_l, self.ps12_l, self.ps13_l]
                self.ps2_l = self.ps20_l  # [self.ps20_l, self.ps21_l, self.ps22_l, self.ps23_l]
                self.ps3_l = self.ps30_l
                #print("self.ps0_l",self.ps0_l[0].size())
                #print("self.ps3_l",self.ps3_l[0].size())
    ...
    def forward(self, image_global, patches, top_lefts, ratio, templates=None, mode=None, global_model=None,
        ...
        if mode == 1:
            c2_g, c3_g, c4_g, c5_g = self.resnet_global.forward(
                image_global)  # self.resnet_global.forward(image_global)
        ...
        if mode == 2:
            ...
            with torch.no_grad():
                if self.patch_n == 0:
                    ...
                    global_feature = self.fpn_global.forward(self.c2_g, self.c3_g, self.c4_g, self.c5_g, mode=1)
                    bfp_global_inputs=[global_feature[0],global_feature[1],global_feature[2],global_feature[3]]

                    global_bfp=self.bfp_collaborative(bfp_global_inputs)


                    self.ps0_g = global_bfp[0] #global_feature[0]
                    self.ps1_g = global_bfp[1] #global_feature[1]
                    self.ps2_g = global_bfp[2] #global_feature[2]
                    self.ps3_g = global_bfp[3] #global_feature[3]
        ...
        if mode == 3:
            ...
            global_fixed = GLNET_FPN_COLLABORATIVE_BFP(in_channels=[256, 512, 1024, 2048], out_channels=256, num_outs=5)
            ...
            #print("load state from global_gl_ga_bfp_4 to extract basic global feature")
            trained_partial = torch.load("../workdir/global_gl_ga_0.5/epoch_50.pth")
            trained_partial_item = trained_partial.get("state_dict")  # 'neck.resnet_global.conv1.weight'
            from collections import OrderedDict
            new_state_dict = OrderedDict()  # 新建一个model
            for k, v in trained_partial_item.items():
            ...
            feat_fusion = self.fpn_global.forward(c2_g, c3_g, c4_g, c5_g,
                                                  c2_ext=self.c2_l, c3_ext=self.c3_l,
                                                  c4_ext=self.c4_l, c5_ext=self.c5_l, mode=3)

            # print("feat_fusion[0].size()",feat_fusion[0].size())
            # print("self.ps0_l[0].size()",self.ps0_l[0].size())

            # print("torch.cat(feat_fusion[0], self.ps0_l[0])",torch.cat((feat_fusion[0], self.ps0_l[0]),dim=1).size())

            self.ps3_l = self.ps30_l

            ''' 
            print("feat_fusion[0]",feat_fusion[0].size())
            print("feat_fusion[1]", feat_fusion[1].size())
            print("feat_fusion[2]", feat_fusion[2].size())
            print("feat_fusion[3]", feat_fusion[3].size())
            print("self.ps0_l[0]",self.ps00_l.size())
            print("self.ps1_l[0]", self.ps10_l.size())
            print("self.ps2_l[0]", self.ps20_l.size())
            print("self.ps3_l[0]", self.ps30_l.size())

            
            inputs = [torch.cat(  (feat_fusion[0], self.ps00_l),dim=1),
                    torch.cat( (feat_fusion[1], self.ps10_l), dim=1),
                    torch.cat( (feat_fusion[2], self.ps20_l),dim=1),
                    torch.cat(  (feat_fusion[3], self.ps30_l),dim=1),]
            ...
            feat=[ensemble_local_fusion_0,
                  ensemble_local_fusion_1,
                  ensemble_local_fusion_2,
                  ensemble_local_fusion_3,
                  F.max_pool2d(ensemble_local_fusion_3,1, stride=2)]



            feat=[self.ensemble_fpn_0(outputs[0]),
            self.ensemble_fpn_2(outputs[1]),
            self.ensemble_fpn_3(outputs[2]),
            self.ensemble_fpn_3(outputs[3]),
            F.max_pool2d(self.ensemble_fpn_3(outputs[3]), 1, stride=2)]
            
            
            

            inputs = [ensemble_local_fusion_0,
                      ensemble_local_fusion_1,
                      ensemble_local_fusion_2,
                      ensemble_local_fusion_3]
            feat = [outputs[0],
                    ...
                    outputs[2],
                    outputs[3],
                    F.max_pool2d(outputs[3], 1, stride=2)]
            ...
            #print("for bfp", self.ps00_l.size())
            #print("for bfp", self.ps30_l.size())
            #print("for bfp value ", self.ps30_l)

            outputs = self.bfp(inputs)

            ensemble_fusion_0 = self.ensemble_fpn_0(outputs[0])
            ensemble_fusion_1 = self.ensemble_fpn_1(outputs[1])
            ensemble_fusion_2 = self.ensemble_fpn_2(outputs[2])
            ensemble_fusion_3 = self.ensemble_fpn_3(outputs[3])

            feat = [ensemble_fusion_0,
                    ensemble_fusion_1,
                    ensemble_fusion_2,
                    ensemble_fusion_3,
                    F.max_pool2d(ensemble_fusion_3, 1, stride=2)]
            '''



            ensemble_local_fusion_0 = self.ensemble_fpn_0(torch.cat((feat_fusion[0], self.ps00_l), dim=1))
            ensemble_local_fusion_1 = self.ensemble_fpn_1(torch.cat((feat_fusion[1], self.ps10_l), dim=1))
            ensemble_local_fusion_2 = self.ensemble_fpn_2(torch.cat((feat_fusion[2], self.ps20_l), dim=1))
            ensemble_local_fusion_3 = self.ensemble_fpn_3(torch.cat((feat_fusion[3], self.ps30_l), dim=1))
