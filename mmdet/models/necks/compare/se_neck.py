...
class SENeck(nn.Module):
    def __init__(self,
        ...
        self.cbam1 = SELayer(mid_channels[2])
        self.cbam2 = SELayer(mid_channels[1])
        if self.use_path_augment:
            self.cbam3 = SELayer(mid_channels[0])
            self.cbam4 = SELayer(mid_channels[1])
            self.cbam5 = SELayer(mid_channels[2])
...
if __name__ == '__main__':
    ...
    neck = SENeck(in_channels=[96, 192, 384, 768], out_channels=256, use_path_augment=False)
    y = neck(x)
    print(y)
