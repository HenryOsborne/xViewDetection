...
from mmdet.models.necks.TransNeck import TransEncoder, CBAM, CoordAtt, AFSM
from mmdet.models.builder import NECKS


@NECKS.register_module()
class TransFPN_AFSM(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
        ...
        super(TransFPN_AFSM, self).__init__()
        ...
        self.coord_att_1 = AFSM(mid_channels[2])
        self.coord_att_2 = AFSM(mid_channels[1])
        if self.use_path_augment:
            self.coord_att_3 = AFSM(mid_channels[0])
            self.coord_att_4 = AFSM(mid_channels[1])
            self.coord_att_5 = AFSM(mid_channels[2])
...
if __name__ == '__main__':
    ...
    neck = TransFPN_AFSM(in_channels=[96, 192, 384, 768], out_channels=256, use_path_augment=True, use_dual_head=False)
    y = neck(x)
    print(y)
