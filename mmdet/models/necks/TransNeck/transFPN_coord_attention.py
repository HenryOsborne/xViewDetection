...
from mmdet.models.necks.TransNeck import TransEncoder, CBAM, CoordAtt
from mmdet.models.builder import NECKS


@NECKS.register_module()
class TransFPN(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
        ...
        self.coord_att_1 = CoordAtt(mid_channels[2], mid_channels[2])
        self.coord_att_2 = CoordAtt(mid_channels[1], mid_channels[1])
        if self.use_path_augment:
            self.coord_att_3 = CoordAtt(mid_channels[0], mid_channels[0])
            self.coord_att_4 = CoordAtt(mid_channels[1], mid_channels[1])
            self.coord_att_5 = CoordAtt(mid_channels[2], mid_channels[2])
...
if __name__ == '__main__':
    ...
    neck = TransFPN(in_channels=[96, 192, 384, 768], out_channels=256, use_path_augment=True, use_dual_head=False)
    y = neck(x)
    print(y)
