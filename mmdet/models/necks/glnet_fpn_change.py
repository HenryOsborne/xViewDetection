...
class fpn_module_global(nn.Module):
    ...
    def forward(self, c2, c3, c4, c5, c2_ext=None, c3_ext=None, c4_ext=None, c5_ext=None, ps0_ext=None, ps1_ext=None,
        ...
        '''
        if mode==1 or mode==3:
            p6 = F.max_pool2d(ps0[0], (1, 1), (2, 2))
            return [ps0[3],ps0[2],ps0[1],ps0[0],p6]
        '''
        # Smooth
        if ps0_ext is None:
        ...
        if mode==1 or mode==3:
            ...
            return [ps2[3],ps2[2],ps2[1],ps2[0],p6]
        # Classify
        '''
        if ps2_ext is None:
...
class fpn_module_local(nn.Module):
    def __init__(self, numClass):
        ...
        self.classify = nn.Conv2d(128 * 4, numClass, kernel_size=3, stride=1, padding=1)
        #local only
        self.toplayer_local = nn.Conv2d(2048 * self.local_flod, 256, kernel_size=1, stride=1, padding=0)  # Reduce channels
        # Lateral layers [C]
...
class GLNET_fpn(nn.Module):
    def __init__(self, numClass):
        ...
        # self._up_kwargs = {'mode': 'bilinear'}
        # Res net
        #self.resnet_global = ResNet(depth=50,num_stages=4,out_indices=(0, 1, 2, 3),frozen_stages=1)
        #self.resnet_local = ResNet(depth=50,num_stages=4,out_indices=(0, 1, 2, 3),frozen_stages=1)

        #self.resnet_global=builder.build_backbone(ResNet)
        #self.resnet_local = builder.build_backbone(ResNet)
        #self.resnet_global.init_weights('torchvision://resnet50')
        ...
        self.fpn_fixed = fpn_module_global(numClass)
        self.renet_global=None
        self.renet_local=None
    ...
    def init_weights(self):
        partial = torch.load("../work_dir/GL_FasterRCNN/epoch_50.pth")
        partial_item = partial.get("state_dict")
        # print({k: v for k, v in partial_item.items() if "backbone" in k})
        state = self.fpn_fixed.state_dict()
        # print(partial_item)
        # 1. filter out unnecessary keys
        # pretrained_dict = {k: v for k, v in partial.items() if k in state}  # and "global" not in k}
        pretrained_dict = {k[16:]: v for k, v in partial_item.items() if "fpn_global" in k}

        # 2. overwrite entries in the existing state dict
        state.update(pretrained_dict)
        # 3. load the new state dict
        self.fpn_fixed.load_state_dict(state)
    ...
    def collect_local_fm(self,c2g,c3g,c4g,c5g, patches, ratio, top_lefts, oped, batch_size, global_model=None,
        ...
        with torch.no_grad():
            ...
            if self.patch_n == 0:
                ...
                self.c2_g, self.c3_g, self.c4_g, self.c5_g=c2g,c3g,c4g,c5g
                #self.output_g

                self.ps0_g, self.ps1_g, self.ps2_g = self.fpn_fixed.forward(
    ...
    def forward(self, c2, c3, c4, c5, patches, top_lefts, ratio, templates=None,
                mode=None,global_image=None, n_patch=None,i_patch=None,model_global=None,model_local=None):
        if mode == 1:
            # train global model
            #c2_g, c3_g, c4_g, c5_g = self.resnet_global.forward(image_global)
            #output_g, ps0_g, ps1_g, ps2_g, ps3_g = self.fpn_global.forward(c2_g, c3_g, c4_g, c5_g,mode=1)

            feat= self.fpn_global.forward(c2, c3, c4, c5, mode=1)
            # imsize = image_global.size()[2:]
            # output_g = F.interpolate(output_g, imsize, mode='nearest')

            return feat
        elif mode == 2 or mode==4:
            #self.patch_n=i_patch
            # train global2local model
            show_feature=True
            #with torch.no_grad():
            if self.patch_n == 0:
                #print(self.fpn_global.state_dict())
                ...
                self.c2_g, self.c3_g, self.c4_g, self.c5_g=model_global
                # # output, ps0, ps1, ps2, ps3  #self.output_g,
                with torch.no_grad():
                    self.ps0_g, self.ps1_g, self.ps2_g = self.fpn_global.forward(self.c2_g,self.c3_g,
                                                                                            self.c4_g,
                                                                                            self.c5_g)
                ...
                if show_feature == True:
                    ...
                    feature=self.ps2_g[0][0][0].cpu().numpy()
                    feature = 1.0 / (1 + np.exp(-1 * feature))
                    feature = np.round(feature * 255)
                    img = transforms.ToPILImage()(feature).convert('RGB')
                    img = img.resize((800, 800), Image.ANTIALIAS)
                    img.save('./feature/' + str(0 + 2) + '.jpg')

            #print("patch_n ", self.patch_n)
            ...
            #output_l, ps0_l, ps1_l, ps2_l, ps3_l
            ps3_l= self.fpn_local.forward(c2, c3, c4, c5,
            ...
            for k, v in partial_item.items():
                if 'neck' in k:
                    name = "module" + k[4:]  # 改为"neck.0"，和local model里的参数命名一致

                    new_state_dict[name] = v

            pretrained_dict = {k: v for k, v in new_state_dict.items() if k in state}
            # print({k for k, v in partial_item.items() })
            # 2. overwrite entries in the existing state dict
            state.update(pretrained_dict)
            # print(len({k for k, v in new_state_dict.items() if 'local' in k}))
            # 3. load the new state dict
            global_fixed.load_state_dict(state)
            global_fixed.eval()
            '''
            self.resnet_global=model_global
            self.resnet_local=model_local
            i_patch=0
            while i_patch < len(top_lefts[0]):


                self.collect_local_fm(c2, c3, c4, c5,
            ...
            c2_g, c3_g, c4_g, c5_g = self.resnet_global.forward(global_image)
            ...
            from PIL import Image
            from torchvision import transforms
            feature = feat[0][0][0].cpu()
            feature = 1.0 / (1 + np.exp(-1 * feature))
            feature = np.round(feature * 255)
            img = transforms.ToPILImage()(feature).convert('RGB')
            img = img.resize((800, 800), Image.ANTIALIAS)
            img.save('./feature/' + str(2) + '.jpg')
