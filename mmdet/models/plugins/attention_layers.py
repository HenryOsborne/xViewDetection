...
class ScaleAwareLayer(nn.Module):
    # Constructor
    def __init__(self, s_size):
        super(ScaleAwareLayer, self).__init__()

        # Average Pooling
        self.avg_layer = nn.AvgPool2d(kernel_size=3, stride=1, padding=1)

        # 1x1 Conv layer
        self.conv = nn.Conv2d(in_channels=s_size, out_channels=1, kernel_size=(1, 1))
...
class DyReLUA(nn.Module):
    ...
    def forward(self, F_tensor):
        ...
        self.init_values = self.init_values.to(device)
...
class TaskAwareLayer(nn.Module):
    # Defining constructor
    def __init__(self, num_channels):
        super(TaskAwareLayer, self).__init__()
