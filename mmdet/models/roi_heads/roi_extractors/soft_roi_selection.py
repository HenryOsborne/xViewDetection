...
class SoftRoIExtractor(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_attention_conv = nn.Sequential(
            nn.Conv2d(out_channels * len(featmap_strides), out_channels, (1, 1)),
            nn.ReLU(),
            nn.Conv2d(out_channels, len(featmap_strides), (3, 3), padding=(1, 1)))
    ...
    def forward(self, feats, rois):
        ...
        for i in range(num_levels):
            roi_feats += (torch.sigmoid(spatial_attention_map[:, i, None, :, :]) * roi_feats_list[i])
        return roi_feats
